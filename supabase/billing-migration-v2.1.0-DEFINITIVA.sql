/* ============================================
   BILLING MODULE - MIGRATION COMPLETA v2.1.0
   Ejecutar en: Supabase Dashboard > SQL Editor
   
   Crea TODAS las tablas necesarias desde cero.
   tenants.id es tipo TEXT -> todas las FK usan TEXT.
   ============================================ */

-- 1. Add code column to products if not exists
ALTER TABLE products ADD COLUMN IF NOT EXISTS code VARCHAR(25);

-- 2. Create business_profiles table (DESDE CERO)
...
-- 4. Drop old invoice tables if they exist
DROP TABLE IF EXISTS invoice_items CASCADE;
DROP TABLE IF EXISTS invoices CASCADE;

-- 5. Create invoices table (SRI Ecuador compliant)
...
-- 7. Enable RLS on all billing tables
ALTER TABLE business_profiles ENABLE ROW LEVEL SECURITY;
ALTER TABLE customers ENABLE ROW LEVEL SECURITY;
ALTER TABLE invoices ENABLE ROW LEVEL SECURITY;
ALTER TABLE invoice_items ENABLE ROW LEVEL SECURITY;

-- 8. RLS Policies (drop first to avoid duplicates)
DROP POLICY IF EXISTS "business_profiles_tenant_policy" ON business_profiles;
...
CREATE POLICY "business_profiles_tenant_policy" ON business_profiles
  FOR ALL USING (
...
SELECT 'v2.1.0 COMPLETE - business_profiles, customers, invoices, invoice_items CREADAS' as result;
