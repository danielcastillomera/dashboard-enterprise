...
import { FileText, Plus, Search, Eye, Download, X, Trash2, DollarSign, CheckCircle2, Code, Printer } from "lucide-react";
...
/* ---- Types ---- */
interface Customer { id: string; tipoIdentificacion: string; identificacion: string; razonSocial: string; email?: string; direccion?: string; }
interface Product { id: string; code?: string; name: string; price: number; stock: number; }
interface LineItem { codigoPrincipal: string; descripcion: string; cantidad: number; precioUnitario: number; descuento: number; ivaCode: string; }
interface Invoice { id: string; invoiceNumber: string; fechaEmision: string; estado: string; importeTotal: number; subtotalSinImpuestos: number; iva15: number; totalDescuento: number; formaPagoDescripcion?: string; claveAcceso?: string; emailSent: boolean; xmlGenerado?: string; customer?: Customer; }
...
export default function FacturacionPage() {
  ...
  useEffect(() => { if (showForm) setDirty(); else clearDirty(); }, [showForm, setDirty, clearDirty]);
  const [showPreview, setShowPreview] = useState(false);
  const [viewPdfUrl, setViewPdfUrl] = useState<string | null>(null);
  const [viewXml, setViewXml] = useState<{ number: string; xml: string } | null>(null);
  ...
  const addItem = () => setItems([...items, { codigoPrincipal: "", descripcion: "", cantidad: 1, precioUnitario: 0, descuento: 0, ivaCode: "4" }]);
  const removeItem = (idx: number) => setItems(items.filter((_, i) => i !== idx));
  const updateItem = (idx: number, field: string, value: string | number) => setItems(items.map((item, i) => i === idx ? { ...item, [field]: value } : item));
  const addProductLine = (p: Product) => setItems([...items, { codigoPrincipal: p.code || p.id.slice(0, 8).toUpperCase(), descripcion: p.name, cantidad: 1, precioUnitario: p.price, descuento: 0, ivaCode: "4" }]);

  const totals = useMemo(() => {
    let sub15 = 0, sub0 = 0, desc = 0;
    for (const item of items) { const line = item.cantidad * item.precioUnitario - item.descuento; desc += item.descuento; if (item.ivaCode === "4") sub15 += line; else sub0 += line; }
    const subSinImp = sub15 + sub0;
    const iva = Math.round(sub15 * 0.15 * 100) / 100;
    return { sub15, sub0, subSinImp, desc, iva, total: Math.round((subSinImp + iva) * 100) / 100 };
  }, [items]);
  ...
  const handleCreate = async () => {
    ...
    setSaving(true);
    try {
      const res = await fetch("/api/invoices", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ customerId: selectedCustomer, formaPago, items, notes }) });
      ...
      setShowForm(false); setItems([]); setSelectedCustomer(""); setNotes("");
      invalidateCache("/api/invoices"); mutateInv();
    } catch (err) { addToast({ message: err instanceof Error ? err.message : "Error", variant: "error" }); }
    finally { setSaving(false); }
  };

  /* ---- Action handlers ---- */
  const handleViewPdf = (inv: Invoice) => setViewPdfUrl(`/api/invoices/${inv.id}?format=pdf`);
  const handleDownloadPdf = (inv: Invoice) => { const a = document.createElement("a"); a.href = `/api/invoices/${inv.id}?format=pdf`; a.download = `factura-${inv.invoiceNumber}.pdf`; a.click(); };
  const handlePrint = (inv: Invoice) => { const w = window.open(`/api/invoices/${inv.id}?format=pdf`, "_blank"); if (w) { w.onload = () => { w.print(); }; } };
  const handleViewXml = async (inv: Invoice) => {
    try {
      const res = await fetch(`/api/invoices/${inv.id}?format=xml`);
      const xml = await res.text();
      setViewXml({ number: inv.invoiceNumber, xml });
    } catch { addToast({ message: "Error al cargar XML", variant: "error" }); }
  };

  const filtered = invoices.filter(inv => (inv.invoiceNumber || "").includes(search) || (inv.customer?.razonSocial || "").toLowerCase().includes(search.toLowerCase()));
  ...
  const columns: Column<Invoice>[] = [
    ...
    { key: "estado", header: "Estado", sortValue: i => i.estado, render: i => { const c: Record<string,string> = { EMITIDA: "bg-green-500/10 text-green-500", CREADA: "bg-yellow-500/10 text-yellow-500", AUTORIZADA: "bg-blue-500/10 text-blue-500" }; return <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${c[i.estado] || "bg-gray-500/10 text-gray-500"}`}>{i.estado || "—"}</span>; } },
    {
      key: "actions", header: "Acciones", sortable: false,
      render: i => (
        <div className="flex gap-0.5 items-center">
          <button onClick={() => handleViewPdf(i)} className="p-1.5 rounded hover:bg-[var(--color-brand-500)]/10 text-[var(--color-brand-500)]" title="Ver PDF"><Eye size={14} /></button>
          <button onClick={() => handleDownloadPdf(i)} className="p-1.5 rounded hover:bg-green-500/10 text-green-600" title="Descargar PDF"><Download size={14} /></button>
          <button onClick={() => handleViewXml(i)} className="p-1.5 rounded hover:bg-blue-500/10 text-blue-500" title="Ver XML"><FileText size={14} /></button>
        </div>
      ),
    },
  ];
  ...
  return (
    <div className="space-y-6">
      <PageHeader title="Panel de Facturación" description="Facturación electrónica SRI Ecuador"
        actions={<button onClick={() => { setShowForm(true); if (items.length === 0) addItem(); }} className="flex items-center gap-2 px-4 py-2.5 rounded-lg bg-[var(--color-brand-500)] text-white text-sm font-medium hover:opacity-90"><Plus size={16} /> Nueva Factura</button>}
      />

      {/* STATS — 3 cards (no anuladas per Ecuador law) */}
      {stats && (
      ...
      {/* XML VIEWER MODAL */}
      {viewXml && (
        <div className="fixed inset-0 z-[70] flex items-center justify-center bg-black/60 p-4" onClick={() => setViewXml(null)}>
          <div className="bg-[var(--color-dashboard-surface)] rounded-xl w-full max-w-3xl max-h-[85vh] flex flex-col overflow-hidden" onClick={e => e.stopPropagation()}>
            <div className="flex justify-between items-center px-4 py-3 border-b border-[var(--color-dashboard-border)]">
              <span className="font-semibold text-[var(--color-text-primary)] text-sm">XML SRI — Factura {viewXml.number}</span>
              <div className="flex gap-2">
                <button onClick={() => { const blob = new Blob([viewXml.xml], { type: "application/xml" }); const url = URL.createObjectURL(blob); const a = document.createElement("a"); a.href = url; a.download = `factura-${viewXml.number}.xml`; a.click(); URL.revokeObjectURL(url); }} className="flex items-center gap-1 px-3 py-1.5 rounded-lg bg-green-600 text-white text-xs font-medium hover:bg-green-700"><Download size={13} /> Descargar</button>
                <button onClick={() => setViewXml(null)} className="p-1.5 rounded hover:bg-[var(--color-dashboard-surface-hover)]"><X size={16} className="text-[var(--color-text-muted)]" /></button>
              </div>
            </div>
            <pre className="flex-1 overflow-auto p-4 text-xs font-mono text-[var(--color-text-muted)] bg-[var(--color-dashboard-bg)] whitespace-pre-wrap break-all">{viewXml.xml}</pre>
          </div>
        </div>
      )}
      ...
      {showForm && (
        <Card>
          ...
          {products.length > 0 && (
            <div className="mb-5">
              ...
              <div className="flex gap-2 flex-wrap">
                {products.slice(0, 8).map(p => (<button key={p.id} onClick={() => addProductLine(p)} type="button" className="text-xs px-3 py-1.5 rounded-lg border border-[var(--color-dashboard-border)] hover:bg-[var(--color-brand-500)]/10 hover:border-[var(--color-brand-500)] text-[var(--color-text-primary)] transition-colors">{p.name} — {formatCurrency(p.price, config)}</button>))}
              </div>
            </div>
          )}
          <div className="mb-5">
                  ...
                  {items.map((item, idx) => {
                    ...
                    return (
                      <tr key={idx} className="border-b border-[var(--color-dashboard-border)]/50">
                        ...
                        <td className="py-1.5 px-2"><select value={item.ivaCode} onChange={e => updateItem(idx, "ivaCode", e.target.value)} className={`${inp} text-xs !py-1.5`}>{Object.entries(IVA_CODES).map(([k,v]) => <option key={k} value={k}>{v.label}</option>)}</select></td>
          ...
          <div className="flex flex-col sm:flex-row gap-3 pt-4 border-t border-[var(--color-dashboard-border)]">
            ...
            <button onClick={() => setShowPreview(true)} type="button" className="flex items-center gap-2 px-5 py-2.5 rounded-lg border border-[var(--color-dashboard-border)] text-sm text-[var(--color-text-primary)] hover:bg-[var(--color-dashboard-surface-hover)] transition-colors"><Eye size={16} /> Vista Previa</button>
            <button onClick={() => { setShowForm(false); setItems([]); }} type="button" className="px-5 py-2.5 rounded-lg bg-red-600 text-white text-sm font-semibold hover:bg-red-700 transition-colors">Cancelar</button>
          </div>
        </Card>
      )}
        ...
        <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/50 p-4" onClick={() => setShowPreview(false)}>
          <div className="bg-white rounded-xl max-w-2xl w-full max-h-[90vh] overflow-y-auto p-6 sm:p-8 text-black" onClick={e => e.stopPropagation()}>
            <div className="flex justify-between items-start mb-6">
              <div><p className="text-xs text-gray-500 font-mono">VISTA PREVIA — NO ES DOCUMENTO FISCAL</p><h2 className="text-lg font-bold mt-1 tracking-widest">F A C T U R A</h2></div>
              <button onClick={() => setShowPreview(false)} className="p-1"><X size={20} /></button>
            </div>
            {selectedCust && (<div className="border rounded-lg p-3 mb-4 text-sm"><p><strong>Cliente:</strong> {selectedCust.razonSocial}</p><p><strong>Identificación:</strong> {selectedCust.identificacion}</p>{selectedCust.direccion && <p><strong>Dirección:</strong> {selectedCust.direccion}</p>}</div>)}
            <table className="w-full text-xs mb-4 border-collapse">
              <thead><tr className="border-b-2 border-gray-300"><th className="text-left py-2">Código</th><th className="text-left py-2">Descripción</th><th className="text-right py-2">Cant.</th><th className="text-right py-2">P. Unit.</th><th className="text-right py-2">Desc.</th><th className="text-right py-2">Total</th></tr></thead>
              <tbody>{items.map((item, idx) => (<tr key={idx} className="border-b border-gray-200"><td className="py-1.5 font-mono">{item.codigoPrincipal}</td><td className="py-1.5">{item.descripcion}</td><td className="py-1.5 text-right">{item.cantidad}</td><td className="py-1.5 text-right">${item.precioUnitario.toFixed(2)}</td><td className="py-1.5 text-right">${item.descuento.toFixed(2)}</td><td className="py-1.5 text-right font-medium">${(item.cantidad * item.precioUnitario - item.descuento).toFixed(2)}</td></tr>))}</tbody>
            </table>
            <div className="flex justify-end"><div className="w-64 text-sm space-y-1">
              ...
              <div className="flex justify-between font-bold text-base border-t border-gray-300 pt-2 mt-2"><span>VALOR TOTAL</span><span>${totals.total.toFixed(2)}</span></div>
            </div></div>
            <div className="mt-4 p-3 bg-gray-50 rounded text-xs text-gray-500 text-center">Forma de pago: {FORMAS_PAGO[formaPago] || formaPago}</div>
          </div>
        </div>
      )}
      ...
      {!showForm && (
        <>
          ...
          <div className="hidden md:block"><Card padding={false}><DataTable columns={columns} data={filtered} emptyMessage="No hay facturas emitidas" caption="Historial de facturas" /></Card></div>
          <div className="md:hidden space-y-3">
            {filtered.length === 0 ? <Card><p className="text-center text-[var(--color-text-muted)] py-4">No hay facturas emitidas</p></Card> : filtered.map(inv => (
              <Card key={inv.id}>
                <div className="flex justify-between items-start mb-2">
                  <span className="font-mono font-medium text-[var(--color-brand-500)] text-sm">{inv.invoiceNumber}</span>
                  <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${inv.estado === "EMITIDA" ? "bg-green-500/10 text-green-500" : "bg-yellow-500/10 text-yellow-500"}`}>{inv.estado}</span>
                </div>
                ...
                <div className="flex gap-2 mt-3 pt-2 border-t border-[var(--color-dashboard-border)]">
                  <button onClick={() => handleViewPdf(inv)} className="text-xs text-[var(--color-brand-500)] hover:underline flex items-center gap-1"><Eye size={12} /> Ver PDF</button>
                  <button onClick={() => handleDownloadPdf(inv)} className="text-xs text-green-600 hover:underline flex items-center gap-1"><Download size={12} /> PDF</button>
                  <button onClick={() => handleViewXml(inv)} className="text-xs text-blue-500 hover:underline flex items-center gap-1"><FileText size={12} /> XML</button>
                </div>
              </Card>
            ))}
          </div>
