...
export default function ReportesPage() {
  ...
  async function handleExportXLSX() { await exportXLSX(reportData as unknown as Record<string, unknown>[], reportExportColumns, `reporte_${new Date().toISOString().split("T")[0]}`); setShowExportMenu(false); }
      ...
      <PageHeader title="Reportes" description="Análisis y métricas del negocio"
        actions={
          <div className="relative">
            ...
            {showExportMenu && (
              <div className="absolute right-0 sm:right-0 top-full mt-1 w-48 max-w-[calc(100vw-2rem)] rounded-xl bg-[var(--color-dashboard-surface)] border border-[var(--color-dashboard-border)] shadow-lg py-1 z-50">
